...
def handle_message(message):
    if message.text == key:
        ...
        bot.send_message(message.chat.id, "^Сегодня наконец-то всё готово. Я заминировал этот чёртов университет и "
                                          ...
                                          "рабочая коробка, всё равно план сработает.^")
        time.sleep(0.5)
        bot.send_message(message.chat.id, "^Если ты это читаешь, то спасибо, ты активировал таймер. Программа взрыва "
                                          "уже запущена. Судя по всему у тебя осталось около 5 минут чтобы всё "
                                          "деактивировать. Правила есть, игрок есть, пусть начнётся ИГРА!^")
    else:
